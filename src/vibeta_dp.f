...
C # To test the python model after compilation
C # Code borrowed from NCL's github repository nfpfort
C # Link to file
C # https://github.com/NCAR/ncl/raw/d7d0692983e0362f20c112a0e2bfe3e26cd1fb26/ni/src/lib/nfpfort/vibeta_dp.f
C import __vibeta__ as vb
C 
C linlog = 1
C # pressure (mb): bottom [near surface] to top
C p = [1000.,925.,850.,700.,600.,500.,
C      400.,300.,250.,200.,150.,100.,
C      70.,50.,30.,20.,10.]
C 
C nlvl   = len(p) 
C psfc   = 1013.
...
C ptop   = 10
C 
C # temperatures (K) at each p level
C x = [292.,285.,283.,277.,270.,260.,
C      250.,235.,225.,215.,207.,207.,
C      213.,220.,225.,228.,230.]
C  
C vint = 0
C ier = 0
C 
C vint, ier = vb.dvibeta(p = p, x = x, xmsg = -999, 
C            linlog = linlog,
C            psfc  = psfc, xsfc = x[0],
C            pbot = pbot, ptop = ptop,
C            plvcrt  = [nlvl-1], 
C            vint = vint, ier = ier)  # units= K-mb
C print("vint="+str(vint))                   # vint ===> vint=255368  K-mb
C 
C vint = vint/(psfc-ptop)                    # normalize (units=K)
C print("vint="+str(vint))                   # vint ===> 254.605 K # Same
C as in the ncl functionv
c
c
...
Cf2py intent(in, out, copy) vint,ier
